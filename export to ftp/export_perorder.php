<?php
include('db.php');
//not used anymore, but to build a unique id
...
foreach ($result as $row => $value) {
    if( $value['exported'] != 1 ){
        $csv_filename = "export_" .$value['reference_id'].".csv"; 
...
//        not tested yet, but to move the uploaded file to a archive location on source location
//        rename("/public_html/export/" .$csv_filename, "/public_html/export/old/" .$csv_filename);
    }//end if exported
}//end foreach
 ?>
